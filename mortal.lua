...
function Library:create(options)
	...
	local watermarkPulse
	watermarkPulse = RunService.Heartbeat:Connect(function(step)
		if not watermark or not watermark.Parent then watermarkPulse:Disconnect(); return end
		local t = tick() % 2
		watermark.TextTransparency = 0.4 + 0.3 * (0.5 + 0.5 * math.sin(t * math.pi))
	end)
	...
	local tabButtons = core:object("ScrollingFrame", {
		Size = UDim2.new(0, 125, 1, -46),
		Position = UDim2.new(1, -130, 0, 41),
		BackgroundTransparency = 1,
		ClipsDescendants = true,
	...
	local sidebarSearch = core:object("TextBox", {
		Size = UDim2.new(0, 115, 0, 22),
	...
	sidebarSearch.FocusLost:connect(function()
		local query = sidebarSearch.Text:lower()
		for _, tabInfo in next, self.Tabs do
			local button = tabInfo[2]
			local name = (tabInfo[3] or ""):lower()
			button.Visible = (query == "" or name:find(query) ~= nil)
		end
	end)
	...
	local minimizeButton = core:object("ImageButton", {
		...
		Theme = {ImageColor3 = "StrongText"},
		Image = "rbxassetid://8498687508",
		AnchorPoint = Vector2.new(1)
	})

	minimizeButton.MouseEnter:connect(function()
		minimizeButton:tween{ImageColor3 = Color3.fromRGB(255, 214, 121)}
	end)
	...
	minimizeButton.MouseButton1Click:connect(function()
		if not minimized then
			...
			core:tween({Size = UDim2.fromOffset(0, 0), Length = 0.25}, function()
				core.Visible = false
			end)
			minimized = true
		else
	...
	creditsTab:credit{Name = "Repository", Description = "UI Library Repository", Github="https://github.com/madium/mortal-lib/blob/master/src.lua"}
